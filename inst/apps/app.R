...
server<-function(input, output) {
   ...
   my_data<-reactive({
     ...
     # essai<-prepare_data(data=data,volume=my_data$volume,rescale=F)
     # model_data<-weightsIMD(data=essai,Y=my_data$volume,X1=c("SEX","AGE0_cr"),
     ...
     newdata_male<-data.frame(time=(seq(input$AGE_VIS[1],input$AGE_VIS[2],length.out=100)-70)/9,
                       ...
                       SEX=rep(0,100),
                       NUM_ID=rep(1,100))
     pred<-predict_WQ(object_boot=boot,
                                   object_rq=rq,
                                   object_cov=boot_cov,newdata=newdata_male,tau=tau,level = 0.05)
     pred_all_male<-data_plot(pred)
     ...
     pred_all_female<-data_plot(pred)

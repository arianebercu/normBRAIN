...
function(data,Y,X1,X2,subject,death,time,interval.death=0,name="weight")
    {
        ...
        prob <- function(wide,Y,X1,X2,subject,death,suivi,delta){
                ...
                if(delta==0)
                    {
                        covar1 <- c(X1,X2)
                        form1 <- formula(paste("R~",paste(covar1,collapse="+"))) # AB : erase factor as only two
                        form1<-as.formula(form1)

                        reg1 <- glm(form1,family="binomial",data=poole,x=T)
                ...
                form2 <- "R~1" # AB : erase factor as only two
                if(length(X1))
                    {
                ...
                reg2 <- glm(form2,family="binomial",data=poole,x=T)
                ...
                return(list(res=res,poole=poole))
            }


        ## fonction pour un delta
        output_reg <- function(wide,Y,X1,X2,subject,death,suivi,delta){
          ...
          {
            covar1 <- c(X1,X2)
            form1 <- formula(paste("R~ 1 +",paste(covar1,collapse="+"))) # AB : erase factor as only two
            form1<-as.formula(form1) # AB : add to detect the formula
          ...
          return(list(reg1=reg1,reg2=reg2))
        }
        ...
        for(delta in interval.death)
            {
                res <- prob(wide=wide_avecobs,Y=Y,X1=X1,
                            ...
                            suivi=time,delta=delta)$res
                poole <- prob(wide=wide_avecobs,Y=Y,X1=X1,
                            X2=X2,subject=subject,death=death,
                            suivi=time,delta=delta)$poole
                reg1<-output_reg(wide=wide_avecobs,Y=Y,X1=X1,
                       X2=X2,subject=subject,death=death,
                       suivi=time,delta=delta)$reg1
                reg2<-output_reg(wide=wide_avecobs,Y=Y,X1=X1,
                                 X2=X2,subject=subject,death=death,
                                 suivi=time,delta=delta)$reg2
                if(delta==0)
                    {
        ...
        return(list(data=data_poids,coef=coef,se=se,reg1=reg1,reg2=reg2,poole=poole))
    }

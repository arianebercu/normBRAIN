...
#' @param nproc
#' @param MPI
#' @import quantreg foreach doParallel stringr
#'
#' @return
#' @export
#'
...
bootwrq_cov <-function(B, form, tau, data, Y, X1=NULL, X2=NULL, subject, death, time, interval.death=NULL, impute=NULL, weight=NULL, wcompute=2, seed=NULL, intermittent, file=NULL, nproc=1, MPI=FALSE)
  {
    ## verif arguments
    ...
    if(!(wcompute %in% c(0,1))) stop("wcompute should be 0, 1 ")
    ...
    if(nproc>1)
    {
      res <- foreach(b=1:B, .combine="+", .errorhandling="remove") %dopar%
        { # for one boostrap return matrix of covariance
          .onewrq_cov(form=form,tau=tau,data=data,Y=Y,X1=X1,
    ...
    {
      res <- foreach(b=1:B, .combine="+", .errorhandling="remove") %do%
        {
          .onewrq_cov(form=form,tau=tau,data=data,Y=Y,X1=X1,
                  X2=X2,subject=subject,death=death,time=time,
                  interval.death=interval.death,impute=impute,
    ...
    res<-res/B
    rownames(res)<-rownames(.onewrq_cov(form=form,tau=tau,data=data,Y=Y,X1=X1,
                                        X2=X2,subject=subject,death=death,time=time,
                                        interval.death=interval.death,impute=impute,
                                        weight=weight,wcompute=wcompute,seed=1,
                                        intermittent=intermittent))
